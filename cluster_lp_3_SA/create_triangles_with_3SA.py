...
splittingPoints = []
epsilon = 0.00
more_dense_range = [[0.28, 0.36, 20], [0.32, 0.75, 10]]
...
splittingPoints += [ 0.01, 0.31, 0.325, 0.62, 0.64, 0.66, 0.68, 0.96, 0.98, 0.99]
splittingPoints += [i / 100 for i in range(32, 36, 1)]

# splittingPoints += [0.05, 0.95, 0.51, 0.49, 0.99]
splittingPoints += [i / 100 for i in range(34, 55, 2)]
...
notInSplittingRange = [0.15, 0.3, 0.45, 0.48, 0.52, 0.55, 0.75, 0.85, 0.95]
...
def createPRange(xRange, yRange, zRange):
	...
	plower = max(0, 1 - (xRange[1] + yRange[1]), 1 - (xRange[1] + zRange[1]), \
		1 - (zRange[1] + yRange[1]), 1- (xRange[1] + yRange[1] +zRange[1]) / 2)
...
def createtriangles(xRange, yRange, zRange, pRange, index):
			...
			for a3 in range(0, computeRatioBase + 1):
				...
				for p in [pl, pu]:
					for i in range(len(EDGES)):
						...
						# if temp == 0.0008231404958676247:
						# 	print("ratio,", x, y, z, p, xRange, yRange, zRange, pRange)
						# if f[i] < temp:
	...
	edges = EDGES[costIdx]
	plower = max(0, 1 - (xRange[1] + yRange[1]), 1 - (xRange[1] + zRange[1]), \
		1 - (zRange[1] + yRange[1]), pRange[0], 1- (xRange[1] + yRange[1] +zRange[1]) / 2)
...
def filter_index(index):
	...
	for item in index:
		...
		if (idx, idy, idz) not in dup_triangles:
			...
			dup_triangles[(idx, idy, idz)][p] = item
		# if x == 0.33 and y == 0.65 and z == 0.99 and item[10] >= 0.02:
		# 	print(item[10], p, dup_triangles[(idx, idy, idz)][p])
		# if item[17] > 0:
		# 	print(item)
	# print(dup_triangles)
...
if __name__ == '__main__':
	...
	# splittingPoints = [0, 0.32, 0.33, 0.65, 0.67, 0.98, 0.99, 1]
	...
	with open('triangles_with_3SA.csv', 'w', newline='') as f:
