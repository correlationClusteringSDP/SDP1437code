...
edgesIndex = {}
epsilon = 0.00
more_dense_range = [[0.45, 0.55, 20], [0.37, 0.65, 10]]

splittingPoints = []

splittingPoints = [0.0, 0.05, 0.1, 0.2, 0.3, 0.31,0.32, 0.33, 0.34, 0.36, 0.38, 0.39, 0.4, \
	 0.42, 0.43, 0.44, 0.46, 0.47, 0.48, 0.49, 0.5, 0.51, 0.52, 0.53, 0.55, \
		0.57, 0.58, 0.59, 0.6, 0.62, 0.63, 0.64, 0.7, 0.8, 0.9, 0.95, 0.97, 0.99, 1.0]
	...
	for a1 in range(0, computeRatioBase + 1):
		for a2 in range(0, computeRatioBase + 1):
			for a3 in range(0, computeRatioBase + 1):
				...
				# Adjust pl and pu: if they are smaller than 1 - (x + y + z) / 2, increase to that value
				p_min = 1 - (x + y + z) / 2
				pl = max(pl, p_min)
				pu = max(pu, p_min)
				if pl > pu:
				...
				for p in p_samples:
					# Since pl and pu have been adjusted to be >= 1 - (x + y + z) / 2,
					# all sampled p values are >= 1 - (x + y + z) / 2, so we can use p directly
...
if __name__ == '__main__':
	...
	with open('pure_cluster_lp_extended_3_SA/triangles.csv', 'w', newline='') as f:
