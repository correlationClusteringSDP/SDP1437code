...
#SBATCH --job-name=sdp_solve
#SBATCH --output=pure_cluster_lp_extended_3_SA_server/slurm_output/sdp_%j.out
#SBATCH --error=pure_cluster_lp_extended_3_SA_server/slurm_output/sdp_%j.err
...
# Default assumes triangles_merged.csv is in pure_cluster_lp_extended_3_SA_server/
TRIANGLES_FILE=${1:-pure_cluster_lp_extended_3_SA_server/triangles_merged.csv}
OUTPUT_DIR=${2:-pure_cluster_lp_extended_3_SA}
...
if [ ! -f "$TRIANGLES_FILE" ]; then
    ...
    echo "Example: sbatch submit_slurm_sdp.sh pure_cluster_lp_extended_3_SA_server/triangles_merged.csv pure_cluster_lp_extended_3_SA"
    exit 1
fi
...
# Run SDP solver
# sdp.py is in pure_cluster_lp_extended_3_SA/ directory
# Redirect stderr to stdout so solver verbose output goes to .out file instead of .err
echo ""
echo "Starting SDP solver..."
python3 pure_cluster_lp_extended_3_SA/sdp.py "$TRIANGLES_FILE" "$OUTPUT_DIR" 2>&1
