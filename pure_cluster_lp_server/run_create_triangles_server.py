...
from pure_cluster_lp.create_triangles import (
...
def initialize_edgesIndex(splittingPoints_list):
    ...
    """
    import pure_cluster_lp.create_triangles as ct_module
...
def generate_range_combinations(splittingPoints_list, range_combinations_file=None, 
    ...
    Files are saved in the server directory (pure_cluster_lp_server)
    A configuration summary is created in the project root directory
    """
    import pure_cluster_lp.create_triangles as ct_module
    
    # Determine project root and server directory
    # This script is in pure_cluster_lp_server/
    script_dir = os.path.dirname(os.path.abspath(__file__))
    server_dir = script_dir  # pure_cluster_lp_server
    ...
    config_file = os.path.join(project_root, 'pure_cluster_lp_server_config.txt')
    
    n = len(splittingPoints_list) - 1
    
    # Initialize edgesIndex before generating combinations
    ...
    with open(config_file, 'w') as f:
        f.write("Server Configuration Summary (pure_cluster_lp)\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write(f"  - range_combinations.txt: pure_cluster_lp_server/range_combinations.txt\n")
        f.write(f"  - splitting_points.txt: pure_cluster_lp_server/splitting_points.txt\n")
        f.write(f"  - edges_index.txt: pure_cluster_lp_server/edges_index.txt\n\n")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1 or (len(sys.argv) == 2 and sys.argv[1] in ['--generate', '-g', 'generate']):
        ...
        print("Files will be saved in: pure_cluster_lp_server/")
        print("Configuration summary will be saved in project root: pure_cluster_lp_server_config.txt")
    ...
    if len(sys.argv) < 3:
        ...
        print("  For standalone mode, run create_triangles.py directly from pure_cluster_lp directory")
        sys.exit(1)
    ...
    if os.path.exists(edges_index_file):
        ...
        # Initialize edgesIndex if file doesn't exist
        initialize_edgesIndex(splittingPoints_list)
