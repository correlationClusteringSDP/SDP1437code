...
#SBATCH --job-name=triangles_range_lp
#SBATCH --output=pure_cluster_lp_server/slurm_output/triangles_range_%A_%a.out
#SBATCH --error=pure_cluster_lp_server/slurm_output/triangles_range_%A_%a.err
...
echo ""
echo "Listing pure_cluster_lp_server directory:"
if [ -d "pure_cluster_lp_server" ]; then
    ls -la pure_cluster_lp_server/ | head -20
else
    echo "  Directory 'pure_cluster_lp_server' not found!"
fi
echo ""
...
# Check if required files exist (in pure_cluster_lp_server directory)
REQUIRED_FILES=("pure_cluster_lp_server/range_combinations.txt" "pure_cluster_lp_server/splitting_points.txt" "pure_cluster_lp_server/edges_index.txt")
MISSING_FILES=()
...
if [ ${#MISSING_FILES[@]} -ne 0 ]; then
    ...
    echo "  cd pure_cluster_lp_server"
    echo "  python3 run_create_triangles_server.py --generate"
    echo "  cd .."
    echo ""
    echo "Or from project root:"
    echo "  cd pure_cluster_lp_server && python3 run_create_triangles_server.py --generate && cd .."
    echo ""
    echo "Current working directory: $(pwd)"
...
python3 pure_cluster_lp_server/run_create_triangles_server.py $TASK_ID $TOTAL_SERVERS pure_cluster_lp_server/range_combinations.txt pure_cluster_lp_server/output_triangles
